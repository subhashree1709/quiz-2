...
answers = ['d', 'a', 'c', 'a', 'b', 'a','a','a','a','b','b','d']

def clientthread(conn):
    score = 0
    conn.send('Welcome to this quiz game!'.encode('utf-8'))
    conn.send('You will receive a question, The ansver to that question should be one of a, b, c or d\n'.encode('utf-8'))
    conn.send('Good Luck!\n\n'.encode("ut(-8"))
        ...
        try:
            ...
            if message:
                if message.lower() == answer:
                    ...
                    conn.send('Incorrect answer! Better luck next tine\n'.encode('utf-8'))
...
def remove(connections):
    if connections in list_of_clients:
        list_of_clients.remove(connections)
